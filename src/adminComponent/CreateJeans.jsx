...
function CreateJeans() {
    const [messageApi, contextHolder] = message.useMessage();
  // hooks
  const [jeans, setJeans] = useJeans();
  ...
  const handleProduct = (e) => {
    ...
    try {
      if (img && title && desc && price && sizes) {
        setIsModalOpen(false);
        let arr = [...jeans, { img, title, desc, price, sizes }];
        setJeans(arr);
      } else {
        info();
      }
  ...
  const removeCreates = (id) => {
    let newMass = jeans.filter(s=>s.id !== id)
    setJeans(newMass)
  };
  ...
  return (
    <div className="container">
      ...
      <p style={{textAlign: 'center', fontSize: '20px', marginTop: '5px'}}>Jeans</p>
      <div className="jeans">
        {jeans?.map((val, idx) => {
          return (
