...
function CreateDressShoes() {
  const [messageApi, contextHolder] = message.useMessage();
  // hooks
  const [dressShoes, setDressShoes] = useDressShoes();
  ...
  const handleProduct = (e) => {
    ...
    try {
      if (img && title && desc && price && sizes) {
        setIsModalOpen(false);
        let arr = [...dressShoes, { img, title, desc, price, sizes }];
        setDressShoes(arr);
      } else {
        info();
      }
  ...
  const removeCreates = (id) => {
    let newMass = dressShoes.filter(s=>s.id !== id)
    setDressShoes(newMass)
  };
...
export default CreateDressShoes;
