...
function CreateSweaters() {
  const [messageApi, contextHolder] = message.useMessage();
  // hooks
  const [sweaters, setSweaters] = useSweaters();
  ...
  const handleProduct = (e) => {
    ...
    try {
      if (img && title && desc && price && sizes) {
        setIsModalOpen(false);
        let arr = [...sweaters, { img, title, desc, price, sizes }];
        setSweaters(arr);
      } else {
        info();
      }
  ...
  const removeCreates = (id) => {
    let newMass = sweaters.filter(s=>s.id !== id)
    setSweaters(newMass)
  };
